...
print("DEBUG: Setting physical_harmonic_orchestrator to None")
        self.physical_harmonic_orchestrator = None
        if HAS_PHYSICAL_HARMONIC_ORCHESTRATOR:
print("DEBUG: Initializing PhysicalHarmonicOrchestrator")
    ...
    def _initialize_quantum_vocabulary_fixed(self):
        ...
        # Criar vocabulário básico se não existir
        if not hasattr(self, 'char_to_idx') or self.char_to_idx is None:
            self.char_to_idx = {}
            for i in range(32, 127):  # Printable ASCII
                self.char_to_idx[chr(i)] = i - 32
        ...
        for i in range(vocab_size):
            # Criar representação baseada no índice do caractere
            char_idx = list(self.char_to_idx.values())[i]
            psi_char = torch.randn(embed_dim, 4, device=self.device) * 0.1
            psi_char[0, 0] = char_idx / 127.0  # Normalizar valor ASCII
            self.quantum_vocab_representations.append(psi_char)

        self.quantum_vocab_representations = torch.stack(self.quantum_vocab_representations)
        print(f"✅ Vocabulário quântico inicializado: {vocab_size} tokens")
    ...
    def _generate_semantic_text(self, psi_final_abstract: torch.Tensor, input_text: str) -> str:
        ...
        """
        # Verificar se já carregamos o modelo semântico
        if not hasattr(self, '_semantic_model_loaded') or not self._semantic_model_loaded:
            try:
                ...
                # Carregar o modelo semântico uma única vez
                self.semantic_model = torch.load(selected_model, map_location=self.device)
    ...
    def _generate_quantum_based_text(self, psi_final_abstract: torch.Tensor, input_text: str) -> str:
        """
        Geração de texto baseada em estados quânticos usando vocabulário quântico
        """
        # Usar o estado quântico para selecionar um token do vocabulário quântico
        if psi_final_abstract.numel() > 0 and hasattr(self, 'quantum_vocab_representations'):
            try:
                # Encontrar token mais próximo no espaço quântico
                if self.quantum_vocab_representations is not None:
                    # Calcular similaridade com todos os tokens no vocabulário
                    similarities = []
                    for token_idx in range(len(self.quantum_vocab_representations)):
                        token_rep = self.quantum_vocab_representations[token_idx]
                        # Calcular similaridade de cosseno
                        sim = torch.nn.functional.cosine_similarity(
                            psi_final_abstract.flatten(),
                            token_rep.flatten(),
                            dim=0
                        )
                        similarities.append((sim.item(), token_idx))
                    ...
                    # Mapear para caractere usando vocabulário nativo se disponível
                    if hasattr(self, 'char_to_idx') and self.char_to_idx:
                        char = list(self.char_to_idx.keys())[best_token_idx % len(self.char_to_idx)]
                        return char
                    else:
                        # Fallback para caractere ASCII
                        char = chr(ord('a') + (best_token_idx % 26))
                        return char

            except Exception as e:
                print(f"      ⚠️  Erro na seleção quântica de token: {e}")

        # Fallback final
        return " "  # Retornar espaço como fallback
