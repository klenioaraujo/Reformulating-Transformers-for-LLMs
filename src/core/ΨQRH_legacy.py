...
# REMOVIDO: NegentropyTransformerBlock foi completamente eliminado
# Este arquivo deve ser refatorado para usar apenas:
# - src.core.spectral_harmonic_processor (processamento espectral puro)
# - src.processing.psiqrh_pipeline (pipeline completo)
# NÃO há fallback - sistema deve falhar claramente se depender de código híbrido

class QRHFactory:
    def __init__(self, config_path: str = "configs/qrh_config.yaml", model_path: Optional[str] = None):
        """
        Inicializa o factory carregando configurações do arquivo YAML.

        Args:
            config_path: Caminho para arquivo de configuração YAML
            model_path: Caminho opcional para modelo pré-treinado (diretório contendo pytorch_model.bin)
        """
        ...
        self.pretrained_model = None  # Modelo pré-treinado (se carregado)
        self.tokenizer = None  # Tokenizer (se modelo pré-treinado carregado)
    ...
    def _load_pretrained_model(self, model_path: str):
        """
        Carrega modelo pré-treinado do diretório especificado.

        Args:
            model_path: Diretório contendo pytorch_model.bin e config.json
        """
        import json
        from pathlib import Path
        ...
        if not model_dir.exists():
            print(f"⚠️  Diretório de modelo não encontrado: {model_path}")
            return

        # Carregar configuração do modelo
        config_file = model_dir / 'config.json'
        if not config_file.exists():
            print(f"⚠️  config.json não encontrado em {model_path}")
            return

        with open(config_file, 'r') as f:
            model_config = json.load(f)

        # Importar PureSpectralTransformer
        try:
            sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
            from train_spectral import PureSpectralTransformer

            # Criar modelo com configuração
            # Usar d_model como fallback para spectral_dim se não existir
            spectral_dim = model_config.get('spectral_dim', model_config.get('d_model', 256))

            self.pretrained_model = PureSpectralTransformer(
            ...
            if checkpoint_file.exists():
                ...
                # Tentar carregar com strict=False para lidar com incompatibilidades
                try:
                    self.pretrained_model.load_state_dict(state_dict, strict=False)
                    self.pretrained_model.eval()
                    print(f"✅ Modelo pré-treinado carregado de {model_path} (strict=False)")
                except Exception as e:
                    print(f"⚠️  Erro ao carregar state_dict: {e}")
                    print(f"⚠️  Continuando sem modelo pré-treinado")
    ...
    def generate_text(self, prompt: str, max_length: int = 50, temperature: float = 1.0, device: str = "cpu") -> str:
        """
        Gera texto usando o modelo pré-treinado (geração autoregressiva).

        Args:
            prompt: Texto inicial para geração
            max_length: Comprimento máximo da geração
            temperature: Temperatura para amostragem (0.0 = determinístico, 1.0+ = criativo)
        ...
        with torch.no_grad():
            for i in range(max_length):
                # Forward pass
                logits = self.pretrained_model(generated)

                # Pegar logits do último token
                next_token_logits = logits[0, -1, :] / temperature

                # Top-k sampling para melhor qualidade (k=10)
                top_k = 10
                ...
                # Parar se atingir newline ou tamanho máximo
    ...
    def _simple_tokenize(self, text: str, device: str) -> torch.Tensor:
        ...
        vocab_file = Path(self.model_path) / "vocab.json" if self.model_path else None

        if vocab_file and vocab_file.exists():
            import json
            with open(vocab_file, 'r') as f:
                vocab = json.load(f)
            # Mapear caracteres para IDs
            char_to_idx = vocab.get('char_to_idx', vocab)
            ids = [char_to_idx.get(c, 0) for c in text]
            return torch.tensor([ids], dtype=torch.long).to(device)
        else:
            # Character-level fallback
            ids = [ord(c) % 34 for c in text]  # Modulo vocab_size
            return torch.tensor([ids], dtype=torch.long).to(device)

    def _simple_decode(self, token_ids: torch.Tensor) -> Optional[str]:
        """Decodificação simples character-level."""
        vocab_file = Path(self.model_path) / "vocab.json" if self.model_path else None
        ...
        if vocab_file and vocab_file.exists():
            ...
            # Usar idx_to_char se disponível
            if 'idx_to_char' in vocab:
                idx_to_char = vocab['idx_to_char']
                # Converter tokens para texto
                chars = []
                for token_id in token_ids.tolist():
                    char = idx_to_char.get(str(token_id), '')
                    if char:
                        chars.append(char)
                return ''.join(chars)
            # Fallback: inverter char_to_idx
            elif 'char_to_idx' in vocab:
    ...
    def process_text(self, text: str, device: str = "cpu") -> str:
        ...
        # Se houver modelo pré-treinado, usar geração de texto
        if self.pretrained_model is not None:
            generated_text = self.generate_text(text, max_length=50, temperature=1.0, device=device)
            return generated_text  # Já retorna string limpa
        ...
        try:
            ...
            # 2. Análise de Consciência Fractal (se disponível) COM ACOPLAMENTO REAL
            consciousness_analysis = ""
            consciousness_results = None
            if self.consciousness_processor is not None:
                try:
                    print("🧠 Iniciando processamento de consciência fractal ACOPLADO...")
                    ...
                    print(f"✅ Tensor de consciência preparado: {consciousness_input.shape}")

                    # ACOPLAMENTO: Extrair spectral_energy e quaternion_phase do enhanced_result
                    if 'consciousness_coupling' in enhanced_result:
                        ...
                        print(f"✅ Dados de acoplamento extraídos: energy={spectral_energy.shape}, phase={quaternion_phase.shape}")
                    else:
                        raise ValueError(
                            "❌ ERRO: consciousness_coupling não encontrado no enhanced_result.\n"
                            "O EnhancedQRHProcessor deve extrair spectral_energy e quaternion_phase."
                        )

                    # Processar através da dinâmica consciente COM ACOPLAMENTO
                    consciousness_results = self.consciousness_processor(
                        consciousness_input,
                        spectral_energy=spectral_energy,
                        quaternion_phase=quaternion_phase
                    )
                    print(f"✅ Processamento de consciência ACOPLADO concluído: {len(consciousness_results)} resultados")
                    ...
                    if isinstance(consciousness_results, dict):
                        ...
                        print(f"✅ Relatório de consciência gerado: {len(consciousness_analysis)} caracteres")
                    else:
                        consciousness_analysis = str(consciousness_results)
                        print(f"⚠️  Resultado de consciência não é dict: {type(consciousness_results)}")
                except Exception as e:
...
"""
    ...
    def generate_gls_output(self, results: Dict[str, Any]) -> Dict[str, str]:
        ...
        if self.consciousness_processor is not None:
            ...
            # Provide fallback GLS generation using basic consciousness metrics
            try:
                from src.conscience.gls_output_generator import GLSOutputGenerator
                generator = GLSOutputGenerator()
                return generator.generate_gls_output(results)
            except Exception as e:
                return {
                    'processing_code': '',
                    'p5js_code': '',
                    'status': 'error',
                    'message': f'Consciousness processor not available and fallback failed: {str(e)}'
                }
