...
class QuaternionRotationLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Aplica rotações SO(4) aos componentes quaterniônicos de forma 100% vetorizada.
        ...
        # Expandir ângulos de rotação para batch e sequência (vetorizado)
        # rotation_angles: [quaternion_dim, 6] -> [1, 1, quaternion_dim, 6] -> [batch, seq, quaternion_dim, 6]
        angles_expanded = self.rotation_angles.unsqueeze(0).unsqueeze(0).expand(batch_size, seq_len, -1, -1)

        # Aplicar rotações SO(4) de forma vetorizada a todos os componentes quaterniônicos
        x_rotated = self.quaternion_ops.so4_rotation(x_quaternion, angles_expanded)  # [batch, seq, quaternion_dim, 4]
...
class DynamicQuantumCharacterMatrix(nn.Module):
    ...
    def __init__(self, vocab_size: int = 50257, hidden_size: int = 256, device: str = "cpu"):
        ...
        # Pré-computar tensores de ressonâncias primas para vetorização
        self.prime_resonances_tensor = torch.tensor(
            [self.prime_resonances[p] for p in self.primes],
            device=self.device
        )
    ...
    def _compute_padilha_quantum_matrix(self, alpha: float, beta: float, D: float) -> torch.Tensor:
        ...
        Versão completamente vetorizada para máximo desempenho.
        ...
        with torch.no_grad():
            ...
            matrix = torch.zeros(self.vocab_size, self.quaternion_dim, 4,
            ...
            # Criar índices vetorizados
            i_idx = torch.arange(self.vocab_size, device=self.device).view(-1, 1)      # [V, 1]
            j_idx = torch.arange(self.quaternion_dim, device=self.device).view(1, -1)  # [1, Q]

            # Coordenadas normalizadas
            base_lambda = (i_idx * j_idx * D) / (self.vocab_size * self.quaternion_dim)  # [V, Q]

            # Modulação por primos (vetorizada)
            prime_modulation = self._apply_prime_modulation_vectorized(base_lambda, i_idx, j_idx)
            lambda_val = base_lambda * prime_modulation

            t = j_idx.float() / self.quaternion_dim

            # Componente complexo da Equação de Padilha
            psi = I0 * torch.sin(omega * t + alpha * lambda_val) * \
                  torch.exp(1j * (omega * t - k * lambda_val + beta * lambda_val**2))

            # Normalizar para quaterniões unitários
            psi_norm = psi / (torch.abs(psi) + 1e-8)

            # Ressonâncias primas vetorizadas
            prime_resonance = self._get_prime_resonance_vectorized(i_idx, j_idx)

            # Componentes quaterniônicos
            w = psi_norm.real
            x = psi_norm.imag * prime_resonance.squeeze(-1)  # Remover dimensão extra
            y = torch.sin(k * lambda_val) * psi_norm.real * prime_modulation
            z = torch.exp(-beta * lambda_val**2) * psi_norm.imag

            # Montar matriz quaterniônica
            matrix[:, :, 0] = torch.complex(w, torch.zeros_like(w))
            matrix[:, :, 1] = torch.complex(x, torch.zeros_like(x))
            matrix[:, :, 2] = torch.complex(y, torch.zeros_like(y))
            matrix[:, :, 3] = torch.complex(z, torch.zeros_like(z))
            ...
            if total_energy.real > 0:
                normalization_factor = torch.sqrt(target_energy / total_energy.real)
                matrix *= normalization_factor
    ...
    def _apply_prime_modulation_vectorized(self, base_value: torch.Tensor, i_idx: torch.Tensor, j_idx: torch.Tensor) -> torch.Tensor:
        ...
        # Criar índices primos vetorizados usando tensor pré-computado
        prime_idx_i = (i_idx.long() % len(self.primes))
        prime_idx_j = (j_idx.long() % len(self.primes))

        # Usar tensores pré-computados para máxima eficiência
        primes_tensor = torch.tensor(self.primes, dtype=torch.float32, device=self.device)
        primes_i = primes_tensor[prime_idx_i]
        primes_j = primes_tensor[prime_idx_j]

        # Usar tensor de ressonâncias pré-computado
        resonances_i = self.prime_resonances_tensor[prime_idx_i]
    ...
    def _get_prime_resonance_vectorized(self, i_idx: torch.Tensor, j_idx: torch.Tensor) -> torch.Tensor:
        ...
        # Calcular índices primos usando tensor pré-computado
        prime_idx = ((i_idx.long() * self.quaternion_dim + j_idx.long()) % len(self.primes))

        # Usar tensor de ressonâncias pré-computado
        resonances = self.prime_resonances_tensor[prime_idx]
    ...
    def _apply_prime_modulation_to_index(self, base_idx: int, position: int) -> float:
        ...
        # Modulação baseada na posição e primo usando tensor pré-computado
        modulation = 1.0 + 0.05 * torch.sin(torch.tensor(base_idx, device=self.device) * resonance).item()
