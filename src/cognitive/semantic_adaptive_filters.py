...
class ContradictionDetector(nn.Module):
    ...
    def detect_contradictions(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # For seq_len=1, std will be NaN, so we use safe computation
        if seq_len > 1:
            ...
            # For single token, use uniform weights
            opposition_weight = torch.ones_like(opposition_norm[:, :1]) * 0.33
            attention_weight = torch.ones_like(attention_norm[:, :1]) * 0.33
            anomaly_weight = torch.ones_like(anomaly_norm[:, :1]) * 0.34
        ...
        # Handle NaN values for very short sequences
    ...
    def apply_contradiction_filter(self, x: torch.Tensor,
        ...
        # Safe std calculation for short sequences
        if seq_len > 1:
            local_std = contradiction_smoothed.std(dim=1, keepdim=True) + self.config.epsilon
        else:
            local_std = torch.ones_like(local_mean) * self.config.epsilon
