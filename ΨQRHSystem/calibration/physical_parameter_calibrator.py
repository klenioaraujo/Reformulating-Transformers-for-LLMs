...
class PhysicalParameterCalibrator:
    ...
    def calibrate_angular_frequency(self, fractal_signal: torch.Tensor) -> float:
        ...
        # Find dominant frequency index (avoid index 0 which is DC component)
        power_spectrum[0] = 0  # Zero out DC component
        dominant_freq_idx = torch.argmax(power_spectrum).item()

        # Ensure index is within bounds
        if dominant_freq_idx >= len(power_spectrum):
            dominant_freq_idx = len(power_spectrum) - 1
        ...
        if dominant_freq_idx < len(freqs):
            ...
            dominant_freq = 0.1  # Fallback frequency
