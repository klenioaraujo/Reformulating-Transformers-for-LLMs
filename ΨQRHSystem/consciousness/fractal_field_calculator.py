...
class FractalFieldCalculator(nn.Module):
    ...
    def compute_field(
        ...
        quaternion_phase: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        Computa campo fractal F(ψ) = -∇V(ψ) + η_fractal(t) + f_wave(ψ,t) + Q(ψ).

        Inclui acoplamento quaterniônico via spectral_energy e quaternion_phase.
        ...
        Args:
            ...
            spectral_energy: Energia espectral quaterniônica [batch, embed_dim] (OPCIONAL)
            quaternion_phase: Fase quaterniônica [batch, embed_dim] (OPCIONAL)
        ...
        # 3. Adicionar componente de onda harmônica f(λ,t) = A*sin(ωt + ϕ0 + θ)
        wave_component = self._compute_wave_component(psi_distribution, time)

        # 4. ACOPLAMENTO QUATERNIÔNICO: modular campo pela energia e fase
        quaternion_modulation = torch.zeros_like(psi_distribution)
        if spectral_energy is not None:
            quaternion_modulation += 0.5 * spectral_energy
        if quaternion_phase is not None:
            quaternion_modulation += 0.3 * torch.sin(quaternion_phase)

        # 5. Combinar para formar campo fractal quaterniônico
        fractal_field = -potential_gradient + fractal_noise + wave_component + quaternion_modulation

        # 6. Aplicar regularização para estabilidade
        ...
        for k in range(1, num_terms + 1):
            if k <= len(lambda_coeffs):
                ...
                if k == 1:
                    ...
                    psi_power = torch.pow(psi + float(self.config.epsilon), k - 1)
    ...
    def _regularize_field(self, field: torch.Tensor) -> torch.Tensor:
        ...
        field = torch.where(
            ...
            field * (max_magnitude / (field_magnitude + float(self.epsilon))),
            field
        )
        ...
        if nan_mask.any():
            ...
            noise_scale = float(self.config.nan_replacement_noise_scale)
            replacement_noise = torch.randn_like(field) * noise_scale
            field = torch.where(nan_mask, replacement_noise, field)

        # 4. Garantir magnitude mínima para evitar zeros completos
        field_magnitude = torch.norm(field, dim=-1, keepdim=True)
        min_magnitude = float(self.config.min_field_magnitude)
        field = torch.where(
    ...
    def _compute_wave_component(self, psi_distribution: torch.Tensor, time: float) -> torch.Tensor:
        """
        Computa componente de onda harmônica: f(λ,t) = A*sin(ωt + ϕ0 + θ)
        ...
        Returns:
            Componente de onda [batch, embed_dim]
        """
        batch_size, embed_dim = psi_distribution.shape

        # Usar índices espaciais como parâmetro λ (posição)
        lambda_positions = torch.arange(embed_dim, dtype=torch.float32, device=self.device)
        lambda_positions = lambda_positions / embed_dim  # Normalizar para [0,1]

        # Expandir para batch
        lambda_param = lambda_positions.unsqueeze(0).expand(batch_size, -1)

        # Parâmetros da equação de onda
        amplitude = 0.05  # Amplitude A (menor para não dominar o campo)
        omega = 2 * math.pi * 0.2  # Frequência angular ω
        phi_0 = math.pi / 3  # Fase inicial ϕ0

        # Fase adaptativa θ baseada na distribuição de probabilidade
        # Usar entropia local como modulação de fase
        safe_psi = psi_distribution + float(self.epsilon)
        log_psi = torch.log(safe_psi)
        entropy_local = -psi_distribution * log_psi
        theta = torch.cumsum(entropy_local, dim=-1) * 0.1  # Fator de escala

        # Equação de onda harmônica: f(λ,t) = A*sin(ωt + ϕ0 + θ)
        wave_component = amplitude * torch.sin(omega * time + phi_0 + theta)

        # Modulação adicional baseada na distribuição
        wave_component = wave_component * (0.5 + 0.5 * psi_distribution)
