#!/usr/bin/env python3
"""
Fix the SpectralQRHLayer inverse transformation
"""
...
def invert_spectral_qrh_fixed(psi_qrh: torch.Tensor, qrh_layer: SpectralQRHLayer) -> torch.Tensor:
    """
    Fixed version of ΨQRH inverse transform.
    """
    batch_size, seq_len, embed_dim, quat_dim = psi_qrh.shape

    # Step 1: Invert quaternion rotations (reverse order)
    ...
    # Invert right rotation first (reverse of forward pass)
    q_right_inv_expanded = q_right_inv.unsqueeze(0).expand(psi_flat.size(0), -1)
    psi_unrotated_right = QuaternionOperations.multiply(psi_flat, q_right_inv_expanded)

    # Invert left rotation
    q_left_inv_expanded = q_left_inv.unsqueeze(0).expand(psi_flat.size(0), -1)
    psi_unrotated = QuaternionOperations.multiply(q_left_inv_expanded, psi_unrotated_right)
...
def test_fixed_inverse():
    """Test the fixed inverse transformation"""
    print("🔍 TESTING FIXED INVERSE TRANSFORMATION")
    print("=" * 60)
    ...
    # Apply fixed inverse
    print("\n🔍 APPLYING FIXED INVERSE TRANSFORM")
    psi_inverted = invert_spectral_qrh_fixed(psi_qrh, qrh_layer)
